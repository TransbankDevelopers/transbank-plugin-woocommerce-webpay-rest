...
class WC_Gateway_Transbank_Oneclick_Mall_REST extends WC_Payment_Gateway_CC
{
    ...
    public function __construct()
    {
        ...
        if ($environment === Options::ENVIRONMENT_PRODUCTION) {
            $this->oneclickInscription->configureForProduction($this->get_option('commerce_code'),
                $this->get_option('api_key'));

            $this->oneclickTransaction->configureForProduction($this->get_option('commerce_code'),
                $this->get_option('api_key'));
        }

        add_action('woocommerce_scheduled_subscription_payment_' . $this->id, [$this, 'scheduled_subscription_payment'],
            10, 3);
    ...
    public function is_valid_for_use()
    {
        return in_array(get_woocommerce_currency(),
            apply_filters('woocommerce_transbank_webpay_oneclick_supported_currencies', ['CLP']), true);
    }
            ...
            <div class="inline error">
                <p>
                    <strong><?php esc_html_e('Gateway disabled',
                            'woocommerce'); ?></strong>: <?php esc_html_e('Oneclick no soporta la moneda configurada en tu tienda. Solo soporta CLP',
                        'transbank_wc_plugin'); ?>
                </p>
            </div>
            <?php
    ...
    public function process_payment($order_id)
    {
        ...
        if (!$order->needs_payment() && !wcs_is_subscription($order_id)) {
            ...
            wc_add_notice(__('Esta transacción puede ya estar pagada o encontrarse en un estado que no permite un nuevo pago. ',
                'transbank_wc_plugin'), 'error');
     ...
     * @throws Oneclick\Exceptions\InscriptionStartException
     */
    public function startInscription(int $order_id = null, string $from = 'checkout'
     ...
     *
     */
    public function authorizeTransaction(WC_Order $order, WC_Payment_Token_Oneclick $paymentToken = null, $amount = null
    ): array {
        if ($paymentToken) {
            $token = $paymentToken;
        } else {
